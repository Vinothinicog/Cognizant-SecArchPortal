...
def init_db():
    ...
    conn.execute('''
        CREATE TABLE IF NOT EXISTS security_reviews (
            ...
            application_id TEXT,
            author_id TEXT,
            field_type TEXT DEFAULT 'application_review',
...
def web_security_assessment(app_id):
    ...
    existing_reviews = conn.execute('''
        ...
        WHERE application_id = ? AND author_id = ?
        ORDER BY created_at DESC
    ''', (app_id, session['user_id'])).fetchall()
    
    for review in existing_reviews:
        if review['field_type'] == 'application_review' and review['status'] in ['submitted', 'completed']:
            app_review_completed = True
        elif review['field_type'] == 'cloud_review' and review['status'] in ['submitted', 'completed']:
